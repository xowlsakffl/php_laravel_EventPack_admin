...
@section('content')
    ...
    @else
        <p>없음</p>
    @endif
    <a href="{{route('admin.layout-navigations.edit', ['londx' => $layoutData->londx])}}">수정</a>
    <form action="{{route('admin.layout-navigations.destroy', ['londx' => $layoutData->londx])}}" method="post">
        @csrf
        @method('DELETE')
